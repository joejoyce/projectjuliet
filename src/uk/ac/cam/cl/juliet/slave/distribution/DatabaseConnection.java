...
/**
 * This encapsulates the database accesses made by the XDPProcessor.
 * It can be mocked to do packet decoding testing.
 * @author lucas
 *
 */
public interface DatabaseConnection {
	 ...
	 * @param orderID 		unique orderID of the new order
	 * @param symbolIndex 	symbolIndex of the stock, determining the order book
	 * @param time_ns		timestamp from the sender in nanoseconds
	 * @param symbolSeqNumber	symbol-sequence-number refers to an EPOCH time stamp
	 * 			in seconds for this stock
	 * @param price			price of the order
	 * @param volume		volume of the order
	 * @param isSell		true if it is sell/ask order and false if it is buy/bid
	 * @param tradeSession	trade sessions for which the order is valid
	 * @throws SQLException
	 */
	 ...
	 * It has no effect to the database in case the order does not exists any more
	 * in the active order book.   
	 * @param orderID 		order ID of the order to be modified
	 * @param symbolIndex 	defines the stock and the order-book in which 
	 * 			the order resides
	 * @param time_ns		timestamp from the sender in nanoseconds
	 * @param symbolSeqNumber	symbol-sequence-number refers to an EPOCH time stamp
	 * 			in seconds for this stock
	 * @param price			potential new price of the order
	 * @param volume		potential new volume of the order
	 * @param isSell		true if it is sell/ask order and false if it is buy/bid
	 * @throws SQLException
	 */
	...
	public void correctTrade(long originalTradeID, long tradeID, long symbolIndex, 
			long time_s, long time_ns, long symbolSeqNumber, long price,
			long volume) throws SQLException;
	/**
	 * Adds a reference time to a stock specified by the symbol index,
	 * the reference time maps to a symbol-sequence-number (other messages
	 * refer to this sequence number to specify a time in seconds)
	 * @param symbolIndex 		specifies the stock the time reference is connected to
	 * @param symbolSeqNumber	sequence number of a sequence that is unique
	 * 			for every symbol index
	 * @param referenceTime		time in seconds (EPOCH) the sequence number maps to
	 * @throws SQLException
	 */
	...
	//TODO: This is a misleading name, it should be removeTrade or addTradeCancellation
